...
pub fn tokenize(file: String) -> Result<Vec<TokenData>> {
    ...
    let mut v = file
        .lines()
        .enumerate()
        .flat_map(|line| parse_line(line.1, line.0).map_err(|err| errors.push(err)))
        .flatten()
...
fn parse_line(line: &str, line_num: usize) -> Result<Vec<TokenData>> {
    ...
    let mut ret: Vec<TokenData> = (0..indentation_level)
        .map(|x| TokenData::new(line_num, x, "\t", Token::Indent))
        .collect();
            ...
            c => {
                ...
                if c.is_numeric() {
                    ...
                    while iter.peek().is_some()
                        && (iter.peek().unwrap().1.is_numeric()
                            || (iter.peek().unwrap_or(&(line.len(), ' ')).1 == '.' && !is_float))
                    {
                        let ch = iter.next().unwrap().1;
                        if ch == '.' {
                            is_float = true;
                    ...
                    while iter.peek().is_some()
                        && (iter.peek().unwrap().1.is_ascii_alphanumeric()
                            || iter.peek().unwrap_or(&(line.len(), ' ')).1 == '_')
                    {
                        let ch = iter.next().unwrap().1;
                        s.push(ch);
                    }
                    ...
                    match s.as_str() {
                        ...
                        _ => ret.push(token_to_tokendata_string!(
                            Token::Literal(TokenLiteral::Identifier(s)),
                            s
                        )),
                    }
                }
