...
use std::{fs::{read_to_string, File}, io::Write}; // Removed Path
use anyhow::Result; // Make sure anyhow is a dependency
...
pub fn run_app() -> Result<()> {
    ...
    match matches.subcommand() {
        Some(("run", submatches)) => {
            let infile_path = submatches.get_one::<String>("IN").ok_or_else(|| anyhow::anyhow!("Missing input file for 'go' command"))?;
        ...
        Some(("compile", submatches)) => {
            let out_file = submatches.get_one::<String>("OUT").ok_or_else(|| anyhow::anyhow!("Missing output file for 'compile' command"))?;
            let in_file = submatches.get_one::<String>("IN").ok_or_else(|| anyhow::anyhow!("Missing input file for 'compile' command"))?;

            println!("Compiling '{}' to '{}'...", in_file, out_file);
            let compiled_code = compile(in_file, debug)?;

            let mut file = File::create(out_file)
                .map_err(|e| anyhow::anyhow!("Failed to create output file '{}': {}", out_file, e))?;
            let serialized = bincode::serialize(&compiled_code)
                 .map_err(|e| anyhow::anyhow!("Failed to serialize bytecode: {}", e))?;
            file.write_all(&serialized)
                 .map_err(|e| anyhow::anyhow!("Failed to write bytecode to file '{}': {}", out_file, e))?;
            println!("Compilation successful.");
        }
        Some(("execute", submatches)) => {
            let in_file = submatches.get_one::<String>("FILE").ok_or_else(|| anyhow::anyhow!("Missing input file for 'run' command"))?;

            println!("Loading bytecode from '{}'...", in_file);
            let file = File::open(in_file)
                 .map_err(|e| anyhow::anyhow!("Failed to open bytecode file '{}': {}", in_file, e))?;
            let compiled_code: Code = bincode::deserialize_from(file)
                 .map_err(|e| anyhow::anyhow!("Failed to deserialize bytecode from '{}': {}", in_file, e))?;
...
pub fn compile(infile_path: &str, debug: u16) -> Result<Code> {
    ...
    if debug & 1 == 1 { // Tokenizer debug flag
    ...
    if let Err(e) = &ast_result {
        ...
        return Err(anyhow::anyhow!("Parsing failed for '{}'", infile_path).context(e.to_string())); // Propagate error with context
    }
    ...
    compiler.compile_from_ast(ast); // Compile the potentially optimized AST
...
pub fn run(code: &Code, debug: u16) -> Result<usize> {
    ...
    let mut machine = ByteCodeMachine::new(code.clone(), vm_debug_mode /*, pass vtables here */);
