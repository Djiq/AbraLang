...
use crate::{tokenizer::TokenLiteral, ByteCode, Expression, Function, Statement, StaticValue, Token};
...
impl Code{
    pub fn string_representation(&self) -> String {
        ...
        for byte in self.bytecode.iter().enumerate() {
            ...
            ret.push_str(&format!("{} | {}:\n",byte.0,serde_json::to_string(&byte.1).unwrap()));
        }
        ret
    }
}
...
impl Compiler{
    ...
    fn compile_body(&mut self,stmts: &Vec<Statement>,additional_variables_to_drop_on_scope_end: Option<&mut Vec<String>>){
        ...
        match expr {
            ...
            Expression::List(literals) => {
                ...
                for literal in literals{
                    match literal {
                        ...
                        crate::TokenLiteral::String(s) => values.push(StaticValue::String(s.clone())),
                        crate::TokenLiteral::Char(c) => values.push(StaticValue::Char(*c)),
                        _ => {}
                    }
                }
                self.bytecode.push(ByteCode::INSTANCE(StaticValue::Array(values)));
            }
            Expression::Literal( literal) => {
                match literal{
                    ...
                    crate::TokenLiteral::Integer(i) =>  self.bytecode.push(ByteCode::PUSH(StaticValue::Integer(*i))),
                    crate::TokenLiteral::Bool(i) =>  self.bytecode.push(ByteCode::PUSH(StaticValue::Bool(*i))),
                    crate::TokenLiteral::Float(i) =>  self.bytecode.push(ByteCode::PUSH(StaticValue::Float(*i))),
                    crate::TokenLiteral::String(s) => self.bytecode.push(ByteCode::PUSH(StaticValue::String(s.clone()))),
                    crate::TokenLiteral::Char(c) => self.bytecode.push(ByteCode::PUSH(StaticValue::Char(*c)))
                }
            },
